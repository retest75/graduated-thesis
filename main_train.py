...
from train.ssl_eval import Evaluation
...
record_path = "/home/chenze/graduated/thesis/record/baseline"
phase = "SL-train"
model_name = "ResNet50"
optimizer_name = "SGD"
...
weight = torch.tensor([0.02, 0.98], device=device)
criterion = FocalLoss(gamma=2, weights=weight)
...
# training
pre_trained = Evaluation(device, model, dataset, dataloader, criterion, optimizer, scheduler)
...
for epoch in range(Epochs):
    epoch_loss, epoch_acc, epoch_fscore, epoch_time = pre_trained.eval_fn(epoch)
    ...
    # save record
    pre_trained.save_log(os.path.join(record_path, "record.log"), phase, epoch, Epochs)

    # print training information for each epoch
    print("=" * 20)
    print(f"Epoch: {epoch+1}/{Epochs} | Loss: {epoch_loss:.6f} | F-1 score: {epoch_fscore:.4f} | Acc: {epoch_acc*100:.3f} | Times: {epoch_time} sec")
    print("=" * 20)

# save best parameter and entire training loss
pre_trained.save_checkpoint(record_path, phase, best_epoch, best_param)
